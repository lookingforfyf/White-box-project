...
#import "NIST_PassWordKeyBoardView.h"
#import "NIST_KeyBoardCharView.h"
#import "NIST_KeyBoardSymbolView.h"
#import "NIST_KeyBoardNumView.h"

#import <AudioToolbox/AudioToolbox.h>

#define plusInch ([UIScreen mainScreen].bounds.size.height > 700)         /* 屏幕判断 */

#define kKeyboardPadding 43                                                /* 密码键盘的宽度 */
...
@interface NIST_PassWordKeyBoardView ()<NIST_KeyBoardCharViewDelegate>
...
@property (nonatomic, strong) NIST_KeyBoardNumView    * numView;
@property (nonatomic, strong) NIST_KeyBoardCharView   * charView;
@property (nonatomic, strong) NIST_KeyBoardSymbolView * symbolView;           /* 符号键盘 */

@property (nonatomic, copy) NSArray                   * numbersDataSource;    /* 字数据源 */
@property (nonatomic, copy) NSArray                   * lettersDataSource;    /* 字母数据源 */

@property (nonatomic, copy) NSArray                   * symbolsDataSource;    /* 符号数据源 */
@property (nonatomic, assign) CGRect                  screenFrame;
@end

@implementation NIST_PassWordKeyBoardView
...
{
    self.numView = [[NIST_KeyBoardNumView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, keyboardHeightOfKeys)];
    self.numView.dataSource = self.numbersDataSource;
    [self.numView initNum];
    __block NIST_PassWordKeyBoardView * blockSelf = self;
...
{
    self.charView = [[NIST_KeyBoardCharView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, keyboardHeightOfKeys)];
...
{
    self.symbolView = [[NIST_KeyBoardSymbolView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, keyboardHeightOfKeys)];
